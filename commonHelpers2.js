import"./assets/burger-menu-5139967a.js";/* empty css                      */import{a as m,P as w,i as d}from"./assets/vendor-61000f44.js";const r={galleryRef:document.querySelector(".gallery"),closeBtn:document.querySelector(".modal_close_btn"),backdrop:document.querySelector(".backdrop"),modalContainer:document.querySelector(".modal-container"),cardRef:document.querySelector(".card")};console.log("refs.modalContainer: ",r.modalContainer);r.cardRef.addEventListener("click",L);async function x(e){return await(await fetch("https://your-energy.b.goit.study/api/exercises/64f389465ae26083f39b17a2")).json()}function L(e){x().then(t=>{k(t)}),r.backdrop.classList.remove("is-hidden"),document.body.classList.add("no-scroll"),r.backdrop.classList.add("scroll"),C()}function C(){document.addEventListener("click",e=>{e.target===r.backdrop&&(r.backdrop.classList.add("is-hidden"),document.body.classList.remove("no-scroll"),r.backdrop.classList.remove("scroll"))}),document.addEventListener("keydown",e=>{e.key==="Escape"&&(r.backdrop.classList.add("is-hidden"),document.body.classList.remove("no-scroll"),r.backdrop.classList.remove("scroll"))}),r.closeBtn.addEventListener("click",()=>{r.backdrop.classList.add("is-hidden"),document.body.classList.remove("no-scroll"),r.backdrop.classList.remove("scroll")})}function k(e){const t=e.gifUrl,o=e.name,s=Number(e.rating).toFixed(1),a=e.target,i=e.bodyPart,n=e.equipment,c=e.popularity,u=e.burnedCalories,E=e.description;return r.modalContainer.innerHTML=`
      <div class="modal-img">
      ...
      </div>`}m.defaults.baseURL="https://your-energy.b.goit.study/api";const q="filters",$="exercises",S="subscription",B=async({page:e=1,perPage:t=12,filter:o="Muscles"}={})=>await m.get(`/${q}?filter=${o}&page=${e}&limit=${t}`).then(s=>s.data),T=async({page:e=1,perPage:t=12,filter:o={}}={})=>await m.get(`/${$}?${new URLSearchParams(o).toString()}&page=${e}&limit=${t}`).then(s=>s.data),M=async()=>{try{const{data:e}=await m.get("/quote");return e}catch{console.log('Error request "/quote"')}},I=async e=>await m.post(`/${S}`,{email:e}).then(t=>t.data);function v(e){return e.toString().padStart(2,"0")}function R(e){return[e.getFullYear(),v(e.getMonth()+1),v(e.getDate())].join("-")}const N=({quote:e,author:t})=>`
